...
setwd("~/ablab/analysis/curio/correlation/gene_count_corr_RDS/")
countData = read.table("all_counts.tsv", header=TRUE, sep="\t", row.names=1 )
...
condition = factor(samplesData[row.names(samplesData) %in% samples,])
...
up_reg_genes <-  subset ( res_sorted , padj < p_adj_cutoff & log2FoldChange > log2_cutoff)$gene_id 
write_tsv(subset ( res_sorted , gene_id %in% up_reg_genes) %>% 
            select(gene_id,log2FoldChange,padj), "upregulated_genes.tsv")
down_reg_genes <-  subset ( res_sorted , padj < p_adj_cutoff & log2FoldChange < - log2_cutoff)$gene_id 
write_tsv(subset ( res_sorted , gene_id %in% down_reg_genes) %>% 
            select(gene_id,log2FoldChange,padj), "downregulated_genes.tsv")
