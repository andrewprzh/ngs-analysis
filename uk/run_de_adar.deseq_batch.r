...
plot_pca_stabilized <- function(stabilizedCounts, samplesData, out_prefix) { 
  # general PCA
  # plot PCA
  pca <- prcomp(t(stabilizedCounts))
  pca_df <- tibble(sample = row.names(pca$x),
                   PC1 = pca$x[, "PC1"], 
                   PC2 = pca$x[, "PC2"], 
                   PC3 = pca$x[, "PC3"]) 
...
batch_correct <- func(countData, samplesData) {
  samples = colnames(countData)
  condition = factor(samplesData[][[1]])
  colData = data.frame(samples=samples, condition=condition)
  summary(colData)
  ddsMatFull = DESeqDataSetFromMatrix(countData=countData, colData=colData, design = ~condition)
  ddsMatFull <- estimateSizeFactors(ddsMatFull)
  ddsMatFull <- estimateDispersions(ddsMatFull)
  vsd <- vst(ddsMatFull, blind = TRUE)
  
  # Extract matrix
  mat <- assay(vsd)
  
  # Remove batch effect
  mat_corrected <- removeBatchEffect(mat, batch = samplesData$batch, design = model.matrix(~condition))
  
  # Plot PCA on corrected data
  plotPCA(DESeqTransform(vsd), intgroup = "condition") # before correction
...
for (ann in c("Ensemble", "RefSeq")) {
  ...
  for (folder in c(folder_prefix, paste0(folder_prefix, "_novel"))) {
    ...
    for (feature_name in c("gene", "transcript")) {
      ...
      condition = factor(samplesData[][[1]])
      colData = data.frame(samples=samples, condition=factor(condition), batch=factor(samplesData$batch))
      summary(colData)
      ddsMatFull = DESeqDataSetFromMatrix(countData=rawData, colData=colData, design = ~batch+condition)
      ddsMatFull <- estimateSizeFactors(ddsMatFull)
      ddsMatFull <- estimateDispersions(ddsMatFull)
      ddsMatFull <- DESeq(ddsMatFull)
      
      plotPCA(vst(ddsMatFull, blind = FALSE), intgroup = c("batch", "condition"))
      
      
      # Example: get results for condition B vs A (replace levels accordingly)
      res_B_vs_A <- results(ddsMatFull, contrast = c("condition", "1B", "CTRL"))
      summary(res_B_vs_A)

      # === RUN DE analysis ===
      for (experiment in c(1, 2, 4, 5)) {
        if (experiment == 1) {
          exp_name = paste0("B1A_vs_Ctrl.", folder)
          countData <- corrected_counts[,c(1,2,3,16,17,18)]
        }
        if (experiment == 2) {
          exp_name = paste0("1B_vs_Ctrl.", folder)
          countData <- corrected_counts[,c(4,5,6,16,17,18)]
        }
        if (experiment == 3) {
          exp_name = paste0("WT_vs_Ctrl.", folder)
          countData <- corrected_counts[,c(7,8,9,16,17,18)]
        }
        if (experiment == 4) {
          exp_name = paste0("ID7_vs_Ctrl.", folder)
          countData <- corrected_counts[,c(10,11,12,16,17,18)]
        }
        if (experiment == 5) {
          exp_name = paste0("B1A_new_vs_Ctrl.", folder)
          countData <- corrected_counts[,c(13,14,15,16,17,18)]
        }
