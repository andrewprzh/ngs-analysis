...
def count_stats(tracking_file, count_dict, transcript_set, bins):
    ...
    for l in open(tracking_file):
        ...
        if transcript_id not in count_dict:
            sys.stderr.write("Counts for transcript %s is not found" % transcript_id)
            continue
...
def main():
    ...
    print("\t".join(map(str, [restored_hist[b] for b in bins])))
    print("\t".join(map(str, [total_hist[b] for b in bins])))
