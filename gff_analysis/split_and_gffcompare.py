...
def parse_args():
    ...
    parser.add_argument("--genedb", "-d", type=str, help="path to the reference annotation;"
                                                         "if no annotation is given input will be simply split")
    parser.add_argument("--gtf", "-g", type=str, required=True, help="gtf to assess")
...
def main():
    ...
    if args.genedb:
        ...
        run_gff_compare(args.genedb, out_full_path, os.path.join(args.output, args.tool + ".full.stats"), "-R")
        print("Running gffcompare for known transcripts")
        run_gff_compare(args.genedb, out_known_path, os.path.join(args.output, args.tool + ".known.stats"), "-R")
        print("Running gffcompare for novel transcripts")
        run_gff_compare(args.genedb, out_novel_path, os.path.join(args.output, args.tool + ".novel.stats"), "-R")
